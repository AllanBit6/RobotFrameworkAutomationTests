...
try:
    from datos_excel import obtener_filas_con_datos
except ImportError:
    from resources.datos_excel import obtener_filas_con_datos
...
class ExpandirCasosExcel(SuiteVisitor):
    def start_suite(self, suite):
        ...
        for test in suite.tests:
            ...
            filas = obtener_filas_con_datos(codigo)
            if not filas:
                tests_expandidos.append(test)
                continue
    ...
    def _configurar_setup(self, test, codigo, fila):
        if test.setup:
            test.setup.config(args=(codigo, str(fila)))
